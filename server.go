...
func NewFromConfig(logger *logrus.Logger, conf Config) (*Server, error) {
	...
	if len(conf.SsfListenAddresses) > 0 {
		...
		if conf.SplunkHecToken != "" && conf.SplunkHecAddress != "" {
			...
			sss, err := splunk.NewSplunkSpanSink(conf.SplunkHecAddress, conf.SplunkHecToken, conf.Hostname, conf.SplunkHecTLSValidateHostname, log, ingestTimeout, sendTimeout, conf.SplunkHecBatchSize, conf.SplunkHecBatchSizeJitter, conf.SplunkHecSubmissionWorkers)
			if err != nil {
				return ret, err
			}
